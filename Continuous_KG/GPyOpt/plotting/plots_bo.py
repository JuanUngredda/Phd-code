...
def plot_acquisition(bounds,input_dim,model,Xdata,Ydata,acquisition_function,suggested_sample, filename = None):
    ...
    if input_dim ==2:
        N_p = 25
        X1 = np.linspace(bounds[0][0], bounds[0][1], N_p)
        X2 = np.linspace(bounds[1][0], bounds[1][1], N_p)
        x1, x2 = np.meshgrid(X1, X2)
        X = np.hstack((x1.reshape(N_p*N_p,1),x2.reshape(N_p*N_p,1)))
        acqu = acquisition_function(X)
        acqu_normalized = -acqu #(-acqu - min(-acqu))/(max(-acqu - min(-acqu)))
        acqu_normalized = acqu_normalized.reshape((N_p,N_p))
        m, v = model.predict(X)
        m=m[0]
        v=v[0]
        ...
        plt.contourf(X1, X2, m.reshape(N_p,N_p),100)
        ...
        plt.subplot(1, 3, 2)
        plt.plot(Xdata[:,0], Xdata[:,1], 'r.', markersize=10, label=u'Observations')
        plt.contourf(X1, X2, np.sqrt(v.reshape(N_p,N_p)),100)
        plt.colorbar()
        plt.xlabel('X1')
        plt.ylabel('X2')
        ...
        plt.colorbar()
        plt.plot(suggested_sample[:,0],suggested_sample[:,1],'k.', markersize=10,color="red")
        plt.xlabel('X1')
        plt.ylabel('X2')
        plt.title('Acquisition function')
        plt.axis((bounds[0][0],bounds[0][1],bounds[1][0],bounds[1][1]))
