...
class maKG(AcquisitionBase):
    ...
    def current_compute_acq(self ):
        ...
        marginal_acqX = self.current_marginal_acq(utility_params_samples)

        if full_support:
            acqX = np.matmul(marginal_acqX, utility_dist)
        acqX = acqX.reshape(-1 )
        return acqX / len(utility_dist)

    def current_marginal_acq(self,  utility_params_samples):
        """
        """
        marginal_acqX = np.zeros(len(utility_params_samples))
       ...
       # Precompute this quantity for computational efficiency.
    ...
    def _compute_acq(self, X , marg_acqX_info=False):
        ...
        if marg_acqX_info == True:
            return X, marginal_acqX
        else:
            if full_support:
    ...
    def _compute_acq_withGradients(self, X):
        ...
        return acqX/len(utility_dist), dacq_dX/len(utility_dist)
                ...
                for l in range(0,len(utility_params_samples)):
                    ...
                    for Z in Z_samples:
                        ...
                        def inner_func(X_inner):
                            ...
                            a = np.reshape(a , (X_inner.shape[0],1))
